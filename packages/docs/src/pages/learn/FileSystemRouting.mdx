...
> **`ssr` is recommended.** Pages are server components rendered through FUNSTACK Router. `ssr: true` is recommended for SEO and faster initial load, but it is no longer required — file-system routing now renders in the dev server (`vite dev`) and production builds with `ssr` either way.
