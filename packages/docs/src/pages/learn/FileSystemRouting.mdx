...
> **`ssr` is optional.** File-system routing works with either `ssr: false` (the default) or `ssr: true`. Pages are server components rendered through FUNSTACK Router; to render them correctly the dev server always server-renders file-system routes regardless of the `ssr` option. Enabling [`ssr`](/api/funstack-static) is still recommended in general for SEO and faster initial load, and controls whether production HTML is pre-rendered.
