...
Because `generateStaticParams` runs on the server at build time, a dynamic route's page module must be a Server Component — it cannot be marked `"use client"` (the build fails with an error naming the offending file). To use a Client Component as the page, move the component body into a separate `"use client"` module and re-export it from the page module:

```tsx
// src/pages/blog/[slug]/page.tsx — a Server Component module
