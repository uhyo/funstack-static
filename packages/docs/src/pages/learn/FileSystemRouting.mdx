...
File-system routing renders pages with FUNSTACK Router, so `@funstack/router` must be installed as a peer dependency:
...
The convention is defined by an **adapter** implementing `FsRoutesAdapter`. Provide a module that `export default`s an adapter to use a different convention:
...
## Userland File-System Routing

If you prefer full control, you can still implement routing in userland with Vite's `import.meta.glob` and the [`entries`](/api/funstack-static) option — deriving [entry definitions](/api/entry-definition) from a glob of page files. The built-in feature is a maintained implementation of this same idea.
