...
`ssr` is optional with `fsRoutes` (the dev server always server-renders file-system routes either way). See [File-System Routing](/learn/file-system-routing) for a full guide.
...
[`ssr`](#ssr-optional) is optional alongside `fsRoutes` and recommended in general (the dev server always server-renders file-system routes regardless of the setting).
...
**Note:** When using [`fsRoutes`](#fsroutes-optional), the dev server always server-renders pages regardless of this setting, so file-system routes render correctly in `vite dev` even with `ssr: false`. The `ssr` option still controls production HTML pre-rendering as described above.
