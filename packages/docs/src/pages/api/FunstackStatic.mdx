...
- **`adapter`** (required) — path to a module that `export default`s an `FsRoutesAdapter`. To use the built-in Next.js-like convention, point this at a module that re-exports `nextRoutes()` from `@funstack/static/fs-routes`.
